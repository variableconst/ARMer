 ...
 * 2011
 */
public class Fuzzy implements Runnable {
     ...
     */
    public void prosesFuzzy(String from, String until, Double minsup, Double mincon, String prefix, Boolean individu, Integer maxitem, Boolean useexisting) {
        this.useexisting = useexisting;
        this.individuSupport = individu;
        this.from = from;
        this.until = until;
    ...
    public List<transaksi> getDataqualified(String from, String until, Integer maxitem) {
        List<transaksi> data = new ArrayList<transaksi>();
        if (from.isEmpty() && until.isEmpty()) {
            data = c.getTransaksiFuzzy(maxitem);
        } else {
            data = c.getTransaksiFuzzyByDate(from, until, maxitem);
        }
        return data;
    }
    ...
    public void hitungSupportCandidate1(List<barang> candidate1) {
        HitungLarge = 0;
        int i = 1;
        int hitungmundur = candidate1.size();
        lg.write("\nJumlah Candidate 1 : " + candidate1.size() + "\nLarge:\n");
        for (barang b : candidate1) {
            hitungmundur--;
            System.out.println(hitungmundur);
            double itemcount = 0;
            double nilaikeanggotaan = 0;
            double mcount = 0;
            List<transaksi> transaksiTemp = c.getTransactionSupport1(b);

            //menghitung nilai keanggotaan
            for (transaksi t : transaksiTemp) {
                double tk = c.getJumlahBarang(t);
                nilaikeanggotaan = 1 / (factorial(tk) / (factorial(1) * factorial(tk - 1)));
                itemcount = itemcount + nilaikeanggotaan;
                mcount = mcount + (1 / tk);
            }

            double support = itemcount / dataqualified.size();
            b.setSupport(support);
            b.setMvalues(mcount);
            if (b.getSupport() >= minsupport) {
                GroupList<barang> temp = new GroupList<barang>();
                temp.add(b);
                temp.setMvalues(b.getMvalues());
                temp.setSupport(b.getSupport());
                large.add(temp);
                System.out.println(b.getKodebarang() + "Jumlah= " + itemcount + "Support= " + b.getSupport() + " " + b.getMvalues());
    ...
    public void hitungSupportCandidate2(List<GroupList<barang>> candidateK, String from, String until, Integer maxitem) {
        HitungLarge = 0;
        int i = 1;
        ...
        for (GroupList<barang> cK : candidateK) {
            ...
            for (transaksi t : transaksiTemp) {
                double tk = c.getJumlahBarang(t);
                nilaikeanggotaan = 1 / (factorial(tk) / (factorial(2) * factorial(tk - 2)));
                itemcount = itemcount + nilaikeanggotaan;
                mcount = mcount + (1 / tk);
            }
            ...
            if (support >= minsupport) {
                ...
                cK.setMvalues(mcount);
                large.add(cK);
                System.out.println(cK.get(0).getKodebarang() + "," + cK.get(1).getKodebarang() + "Support " + support + " " + itemcount);
    ...
    public void hitungSupportCandidateK(List<GroupList<barang>> candidateK, Integer k, String from, String until, Integer maxitem) {
        HitungLarge = 0;
        int i = 1;
        ...
        for (GroupList<barang> cK : candidateK) {
            ...
            for (transaksi t : transaksiTemp) {
                double tk = c.getJumlahBarang(t);
                nilaikeanggotaan = 1 / (factorial(tk) / (factorial(k) * factorial(tk - k)));
                itemcount = itemcount + nilaikeanggotaan;
                mcount = mcount + (1 / tk);
            }
            ...
            if (support >= minsupport) {
                ...
                cK.setSupport(support);
                cK.setMvalues(mcount);
                System.out.println(cK.get(0).getKodebarang() + "," + cK.get(1).getKodebarang() + "," + cK.get(2).getKodebarang() + ", " + support + " " + mcount);
    ...
    public void generateCandidate2(List<GroupList<barang>> large) {
        ...
        candidatecount.add(candidateK.size());
    }
                        ...
                        for (GroupList<barang> antecedent : large) {
                            if (combination.size() == antecedent.size()) {
                                ...
                                if (jumlahbarang == antecedent.size()) {
                                    ...
                                    rules++;
                                    double confidence = (double) listLarge.getMvalues() / (double) antecedent.getMvalues();
//                                    double lift = ruleSupport(listLarge) / (ruleSupport(antecedent) * ruleSupport(consequent));
//                                    double conviction = (1 - ruleSupport(consequent)) / (1 - (ruleSupport(listLarge) / ruleSupport(antecedent)));
//
//                                    if (conviction == Double.POSITIVE_INFINITY) {
//                                        conviction = 0;
//                                    }

                                    System.out.println(confidence + "/" + (double) consequent.getSupport());
                                    System.out.println("antecedent " + listLarge.getMvalues() + "consequent " + antecedent.getMvalues());
...
//                                        lg.write("], Confidence : " + confidence + ", Support : " + listLarge.getSupport() + ", lift : " + lift + ", conviction : " + conviction + "\n");
                                        lg.write("], Confidence : " + confidence + ", Support : " + listLarge.getSupport() + "\n");
                                        hitung++;
                                    }
                                }
                            }
    ...
    public double ruleSupport(GroupList<barang> cK) {
        List<transaksi> transaksiTemp = c.getTransactionSupport(cK);
    ...
    public void run() {
        ...
        if (useexisting == true) {
            ...
            dataqualified = getDataqualified(from, until, maxitem);
        }
        ...
        if (individuSupport) {
            getParameter(AnalysisProsesUI.getSupportIndividuDialog(), minsupport, prefix);
            lg.write("Minimum support 2 : " + minsupport + "\n");
        }
        lg.loading("Support Count Candidate 2");
        hitungSupportCandidate2(candidateK, from, until, maxitem);
        ...
        do {
            ...
            if (individuSupport) {
                getParameter(AnalysisProsesUI.getSupportIndividuDialog(), minsupport, prefix);
                lg.write("Minimum support " + jumlahkombinasi + " : " + minsupport + "\n");
            }
            lg.loading("Support Count Candidate " + jumlahkombinasi);
            hitungSupportCandidateK(candidateK, jumlahkombinasi, from, until, maxitem);
            jumlahkombinasi++;
        } while (loop);
